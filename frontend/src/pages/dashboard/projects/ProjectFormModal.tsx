      ...
      <div className="relative w-full max-w-3xl max-h-[88vh] flex flex-col bg-[#0e1c1c] border border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06] shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">管理项目内容与展示信息</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-none px-6 py-5 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="项目 ID" hint="留空时自动生成 UUID">
              <TextInput value={form.id} onChange={(v) => setField("id", v)} placeholder="自定义 ID 或留空" />
            </FormField>
            <div className="flex items-end gap-3">
              <div className="flex-1">
                <label className="block text-xs font-medium text-gray-400 mb-2">发布状态</label>
                <button
                  type="button"
              ...
              <div className="flex-1">
                <label className="block text-xs font-medium text-gray-400 mb-2">首页精选</label>
                <button
                  type="button"
          ...
          <FormField label="项目标题" required>
            <TextInput value={form.title} onChange={(v) => setField("title", v)} placeholder="输入项目名称" />
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              >
                <option value="" disabled className="bg-[#0e1c1c]">选择分类</option>
                {CATEGORIES.map(c => <option key={c} value={c} className="bg-[#0e1c1c]">{CATEGORY_LABELS[c]}</option>)}
              </select>
            </div>
            <FormField label="创建年份" icon={<Calendar size={12} className="text-gray-500"/>}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="技术栈" icon={<Code size={12} className="text-gray-500"/>} hint="按回车添加">
            ...
            <FormField label="运行平台" icon={<Layers size={12} className="text-gray-500"/>} hint="按回车添加">
              <div className="bg-white/[0.04] border border-white/[0.08] rounded-xl p-2.5 space-y-2 focus-within:border-accentColor/60">
                ...
                <input value={platformInput} onChange={(e) => setPlatformInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addArrayItem("platform_apps", platformInput, setPlatformInput); } }} placeholder="网页端、移动端..." className="w-full bg-transparent text-sm text-gray-200 placeholder:text-gray-600 outline-none" />
              </div>
            </FormField>
          </div>

          <div className="grid grid-cols-2 gap-4">
             <FormField label="在线地址" icon={<Globe size={12} className="text-gray-500"/>}>
               <TextInput value={form.live_url || ""} onChange={(v) => setField("live_url", v)} placeholder="https://..." />
             </FormField>
             <FormField label="GitHub API 地址" icon={<Link2 size={12} className="text-gray-500"/>} hint="用于读取实时星标数">
               <TextInput value={form.github_api || ""} onChange={(v) => setField("github_api", v)} placeholder="https://api.github.com/..." />
             </FormField>
          </div>

          <FormField label="缩略图" icon={<Upload size={12} className="text-gray-500" />}>
            <div className="space-y-2">
              {form.thumbnail_url ? (
                <div className="relative w-full h-36 rounded-xl overflow-hidden border border-white/[0.08] bg-white/[0.02]">
                  <img src={form.thumbnail_url} alt="缩略图预览" className="w-full h-full object-cover" />
          ...
          <div>
            ...
            <div className="bg-white/[0.04] border border-white/[0.08] rounded-xl p-3 space-y-3 focus-within:border-accentColor/60 transition-colors">
              ...
              <div className="flex items-center gap-2 mt-2">
                <input value={ghLabelInput} onChange={(e) => setGhLabelInput(e.target.value)} placeholder="标签（网页、移动端）" className="w-1/3 bg-white/[0.02] border border-white/[0.08] rounded-lg px-3 py-2 text-sm text-gray-200 outline-none focus:border-accentColor/50" />
